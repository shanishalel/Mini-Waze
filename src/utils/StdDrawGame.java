 ...
 *  @author Kevin Wayne
 */
public final class StdDrawGame implements ActionListener, MouseListener, MouseMotionListener, KeyListener {
	...
	private static StdDrawGame std = new StdDrawGame();
...
//		menu2.add(menuItem7);
	...
	private static Image getImage(String filename) {
		...
		if ((icon == null) || (icon.getImageLoadStatus() != MediaTracker.COMPLETE)) {
			URL url = StdDrawGame.class.getResource(filename);
			if (url != null)
				icon = new ImageIcon(url);
		}
...
//		switch(str) {
//		case "Scenario" : MyGameGUI.Scenario();
//		break;
//		default:
//			break;
//		}
	...
	public void mousePressed(MouseEvent e) {
		synchronized (mouseLock) {
			mouseX = StdDrawGame.userX(e.getX());
			mouseY = StdDrawGame.userY(e.getY());
			isMousePressed = true;
		}
	}
	 ...
	 */
	public static void main(String[] args) {
		StdDrawGame.square(0.2, 0.8, 0.1);
		StdDrawGame.filledSquare(0.8, 0.8, 0.2);
		StdDrawGame.circle(0.8, 0.2, 0.2);

		StdDrawGame.setPenColor(StdDrawGame.BOOK_RED);
		StdDrawGame.setPenRadius(0.02);
		StdDrawGame.arc(0.8, 0.2, 0.1, 200, 45);

		// draw a blue diamond
		StdDrawGame.setPenRadius();
		StdDrawGame.setPenColor(StdDrawGame.BOOK_BLUE);
		double[] x = { 0.1, 0.2, 0.3, 0.2 };
		double[] y = { 0.2, 0.3, 0.2, 0.1 };
		StdDrawGame.filledPolygon(x, y);

		// text
		StdDrawGame.setPenColor(StdDrawGame.BLACK);
		StdDrawGame.text(0.2, 0.5, "black text");
		StdDrawGame.setPenColor(StdDrawGame.WHITE);
		StdDrawGame.text(0.8, 0.8, "white text");
	}
